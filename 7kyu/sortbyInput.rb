...
people = [
  ...
  user_category_input = gets.chomp 

  puts 'Enter what value to search for'
  user_value_search = gets.chomp 

  people.each do |find_users|
    if find_users[user_category_input] == user_value_search
        puts find_users['name']
    end
  end
